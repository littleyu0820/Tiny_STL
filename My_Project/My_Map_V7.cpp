...
struct TreeNode
{
	...
	//TreeNode(Key value, TreeNode<Key, Value>* parent) :
...
TreeNode<Key, Value>* MyRBT<Key, Value>::insert_node(TreeNode<Key, Value>* cur_node, Key key, Value value)
{
	...
	TreeNode<Key, Value>* last_node = nullptr;
...
void test()
{
	...
	std::cout << "delta: " << m["delta"] << "\n";     // 4
	std::cout << "epsilon: " << m["epsilon"] << "\n"; // 5
	m["beta"] = 42;
	std::cout << "beta (after overwrite): " << m["beta"] << "\n"; // 42

	// 測試預設插入
	std::cout << "zeta (default insert): " << m["zeta"] << "\n"; // 預期是 0
	m["zeta"] = 6;
	std::cout << "zeta (after assign): " << m["zeta"] << "\n";   // 6
