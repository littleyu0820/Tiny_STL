...
public:
	...
	public:
		...
		T& operator*()
		{
			assert(it_ptr && it_ptr->nextptr && "Error: Dereferencing end() iterator!");
			return it_ptr->data;
		}
		...
		List_Iterator operator--()
		{
			...
			assert(it_ptr && it_ptr->preptr && "Error: Decrementing begin() iterator");
...
private:
	...
	/*ListNode<T>* copy_list(const ListNode<T>* b, ListNode<T>* e)
	{
		MyList<T> temp;
		while (b != e)
			temp.push_back(*b++);
