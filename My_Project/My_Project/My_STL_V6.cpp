...
public:	
	...
	public:
		...
		T* operator&() //we can use & to get the address where iterator is or we said where the iterator point to
		{
			return t_ptr;
		}
	...
	*/
	...
	MyVector& operator=(const MyVector& rhs)&  //going to use swap
	{
		if (this != &rhs)
		{
	...
	MyVector& operator=(MyVector&& rhs) & noexcept //going to use swap
	{
		if (this != &rhs)
		{
			std::cout << "Assigning vector..." << std::endl;
...
private:
	...
	{
		std::swap(the_first, rhs.the_first);
